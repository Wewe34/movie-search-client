        ...
        <Box minHeight={'50vh'} paddingBottom={10}>
            ...
            <Box sx={{display: 'flex', flexDirection: 'column', maxWidth: '100%', paddingX: 1}}>
                <Typography 
                    ...
                    color='primary' 
                    fontWeight={'bold'} 
                    sx={{paddingY:3, paddingLeft:2}} >Movies</Typography>
                <Box sx={{display: 'flex', overflowX:'auto'}}>
                    {list.filter((film) => film.Type === 'movie').map((movie:any, index: number) => {
                               return <Box key={index}>
                                        <MovieStripTape />
                                            <Box 
                                                sx={{display:"flex", flexDirection:"column", padding: 2, backgroundColor:'black',cursor:"pointer", height: '350px'}} 
                                                onClick={() => getSelectionById(movie.imdbID)}>
                                                        <Box component="img"
                                                            sx={{
                                                            maxHeight: { xs: 250},
                                                            maxWidth: { xs: 145},
                                                        ...
                                                        <Typography 
                                                            fontWeight={'bold'} 
                                                            sx={{maxWidth: '135px'}} 
                                                            color="primary">{`${movie.Title.slice(0,50)}${movie.Title.length >= 50 ? '...' : ''}`}</Typography>
                    ...
                    {list.filter((film) => film.Type === 'series').map((series:any, index: number) => {
                          return <Box key={index}>
                                    <MovieStripTape />
                                        <Box 
                                            sx={{display:"flex", flexDirection:"column", padding: 2, backgroundColor:'black',cursor:"pointer", height: '350px'}} 
                                            onClick={() => getSelectionById(series.imdbID)}>
                                                    <Box component="img"
                                                        sx={{
                                                        maxHeight: { xs: 250},
                                                        maxWidth: { xs: 145},
            ...
            <Box sx={{display: 'flex', flexDirection: 'column', maxWidth: '100%', paddingX: 1}}>
                <Typography 
                    ...
                    color='primary' 
                    fontWeight={'bold'} 
                    sx={{paddingY:3, paddingLeft:2}} >Episodes</Typography>
                <Box sx={{display: 'flex', overflowX:'auto'}}>
                    {list.filter((film) => film.Type === 'episode').map((episode:any, index: number) => {
                        return <Box key={index} >
                                    <MovieStripTape />
                                        <Box 
                                            sx={{display:"flex", flexDirection:"column", padding: 2, backgroundColor:'black',cursor:"pointer", height: '350px'}} 
                                            onClick={() => getSelectionById(episode.imdbID)}>
                                                    <Box component="img"
                                                        sx={{
                                                        maxHeight: { xs: 250},
                                                        maxWidth: { xs: 145},
